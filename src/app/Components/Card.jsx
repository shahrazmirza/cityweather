  ...
  const getWeatherData = async (city) => {
    try {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?appid=914cd4190a0cdd48e9112eb8afdbf306&units=metric&q=${city}`
      );
    ...
    <div className="flex flex-col gap-5 justify-center items-center p-10 shadow-2xl border border-cyan-500 w-fit rounded-3xl md:h-auto h-screen">
      ...
      {weatherData ? (
        <>
          <Image
            src={
              weatherData.weather && weatherData.weather[0].main === "Clouds"
                ? clouds
